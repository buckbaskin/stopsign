...
print('K %s' % KLASSIFIER_PATH)
print('R %s' % REDUCER_PATH)
...
if __name__ == '__main__':
    ...
    try:
            ...
            for index, Klassifier in enumerate(Klassifiers):
                ...
                for config_setup in Klassifier_configs:
                    ...
                    for seed in range(0, num_tests):
                        ...
                        reducer = PCA(n_components=17)
                        print('begin fit')
                        start = datetime.datetime.now()
                        shrink_train_X = reducer.fit_transform(train_X)
                        print(reducer.explained_variance_ratio_)
                        classifier.fit(shrink_train_X, train_y)
                        print('end fit | %.1f sec' % ((datetime.datetime.now() - start).total_seconds(),))
                        y_pred = classifier.predict(shrink_train_X)
                        y_pred = np.where(y_pred > 0.5, 1, 0)

                        train_acc_accum += accuracy_score(y_true=train_y, y_pred=y_pred)
                        
                        if FAST_COMPILE:
                            print('training accuracy: %.2f' % (train_acc_accum,))
                            raise ValueError()
                        ...
                        for test_X_sub, test_y_sub in split_version:
                            test_X_sub = reducer.transform(test_X_sub)
                            count += 1
        ...
        print('Fast Compile? %s' % (FAST_COMPILE,))
        if not FAST_COMPILE:
            raise
    ...
    print('I should save these')
    joblib.dump(classifier, KLASSIFIER_PATH)
    print('done saving this silly MLP thing')
    joblib.dump(reducer, REDUCER_PATH)
    print('done saving this magic PCA thing')
